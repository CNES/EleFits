...
std::tuple<FitsIO::VecColumn<Ts>...> readColumns(fitsfile* fptr,
 ...
 */
template <typename... Ts>
void insertColumns(fitsfile* fptr,
    long index,
    const FitsIO::Column<Ts>&... columns);
...
template <typename T>
void initColumnImpl(fitsfile* fptr,
    long index,
...
template <typename T>
void readColumnChunkImpl(fitsfile* fptr,
    long index,
    FitsIO::VecColumn<T>& column,
    long firstRow,
    long rowCount);
...
struct ReadColumnChunksImpl {
  void operator()(fitsfile* fptr,
      ...
      long firstRow,
      long rowCount) {
    readColumnChunkImpl(fptr,
        indices[i],
        std::get<i>(columns),
        firstRow,
        rowCount);
    ReadColumnChunksImpl<i - 1, Ts...> {}(fptr,
        indices,
        columns,
        firstRow,
        rowCount);
...
template <typename T>
void writeColumnChunkImpl(fitsfile* fptr,
    long index,
...
struct WriteColumnChunksImpl {
  void operator()(fitsfile* fptr,
      ...
      long firstRow,
      long rowCount) {
    writeColumnChunkImpl(fptr,
        indices[i],
        std::get<i>(columns),
        firstRow,
        rowCount);
    WriteColumnChunksImpl<i - 1, Ts...> {}(fptr,
        indices,
        columns,
        firstRow,
        rowCount);
...
void writeColumnsImpl(fitsfile* fptr,
...
std::tuple<FitsIO::VecColumn<Ts>...> readColumns(fitsfile* fptr,
  ...
  std::tuple<FitsIO::VecColumn<Ts>...> columns;
  internal::InitColumnsImpl<sizeof...(Ts) - 1, Ts...> {}(fptr,
      indices,
      names,
      columns,
      rows);
  ...
  if (chunkRows == 0)
    throw std::runtime_error(
        "Cannot compute the optimal number of rows to be read at once");
  /* Read column data */
  for (long first = 1; first <= rows; first += chunkRows)
  {
    ...
    internal::ReadColumnChunksImpl<sizeof...(Ts) - 1, Ts...> {}(fptr,
        indices,
        columns,
        first,
        chunkRows);
...
void writeColumns(fitsfile* fptr, const FitsIO::Column<Ts>&... columns) {
  ...
  for (long first = 1; first <= rows; first += chunkRows)
  {
    ...
    if (last > rows)
      chunkRows = rows - first + 1;
    internal::WriteColumnChunksImpl<sizeof...(Ts) - 1, Ts...> {}(fptr,
        indices,
        table,
        first,
        chunkRows);
