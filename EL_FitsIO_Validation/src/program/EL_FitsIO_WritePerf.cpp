...
public:
  ...
  Elements::ExitCode mainMethod(std::map<std::string, variable_value>& args) override {
    ...
    Test::Benchmark* benchmark = nullptr;
    if (testCase == "CFitsIO") {
      benchmark = new Test::CfitsioBenchmark(filename);
    } else if (testCase == "EL_FitsIO_v1") {
      benchmark = new Test::ElUnbufferedBenchmark(filename);
    } else if (testCase == "EL_FitsIO") {
      benchmark = new Test::ElBenchmark(filename);
    }
