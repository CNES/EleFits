...
BOOST_AUTO_TEST_CASE(ctor_test)
{
  ...
  const FileMemRegions<2> region_position(Region<2>::from_shape(file, shape), memory);
  ...
  const FileMemRegions<2> position_region(file, Region<2>::from_shape(memory, shape));
  ...
  const FileMemRegions<2> region(Region<2>::from_shape(file, shape));
