...
BOOST_FIXTURE_TEST_CASE(several_records_test, Fits::Test::MinimalFile) {
  ...
  HeaderIo::write_records(this->fptr, strRecord, boolRecord);
  ...
  checkClose(HeaderIo::parse_record<float>(this->fptr, "FLOAT").value, 3.14F);
  strRecord.value = "NEW";
  boolRecord.value = false;
  std::get<0>(records).value = 43;
  std::get<1>(records).value = 4.14F;
  HeaderIo::update_records(this->fptr, strRecord, boolRecord);
  ...
  checkClose(HeaderIo::parse_record<float>(this->fptr, "FLOAT").value, 4.14F);
}
...
BOOST_FIXTURE_TEST_CASE(parse_vector_and_map_test, Fits::Test::MinimalFile) {
  ...
  HeaderIo::write_records(this->fptr, shortRecord, longRecord, longlongRecord);
